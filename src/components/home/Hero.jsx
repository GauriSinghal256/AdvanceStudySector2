...
export function Hero({ onNavigate }) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setImageIndex((current) => (current + 1) % featureImages.length), 4500)
  ...
  return <section className="hero-cinematic" ref={heroRef}>
    ...
    <div className="hero-content section-wrap">
      <div className="hero-text-block">
        ...
        <Reveal delay={360}><div className="hero-actions"><button className="button button-primary" onClick={() => onNavigate('courses')}>Explore courses <ArrowIcon /></button><button className="button button-ghost" onClick={() => onNavigate('about')}>Discover our story <ArrowIcon /></button></div></Reveal>
