...
export function CommunitySection() {
  return <section className="community-section">
    <div className="section-wrap">
      ...
      <div className="community-grid">{communityImages.map((item, i) => <Reveal key={item.title} delay={i * 120}><Link className="community-card" to="/contact" aria-label={`${item.title}, contact us`}><img src={item.src} alt={item.alt} /><div className="community-overlay"><strong>{item.title}</strong><span>↗</span></div></Link></Reveal>)}</div>
    </div>
  </section>
}
